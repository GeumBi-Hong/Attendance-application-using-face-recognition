...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            token.setActionCallback(new IMqttActionListener() {
                ...
                public void onSuccess(IMqttToken asyncActionToken) {  //연결에 성공한경우
                    ...
                    Log.e("Device_connect_success", "Success");

                    try {
                        mqttAndroidClient.subscribe("kitae", 0);
                ...
                public void onFailure(IMqttToken asyncActionToken, Throwable exception) {   //연결에 실패한경우
                    Log.e("Device_connect_fail", "Failure " + exception.toString());
                }
            });
        ...
        alert.setNegativeButton("취소", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) { //취소 버튼을 클릭 했을 때
            }
        });
        alert.show();
                ...
                if (view.getId() == R.id.video) {
                    new CountDownTimer(10 * 1000, 1) {
                        ...
                        public void onTick(long m) {
                            progressBar.setProgress((int) (100 * 100 - m) / 100);
                        }
...
//        Toast.makeText(MainActivity.this, "Video saved: " + mNextVideoAbsolutePath,
        ...
        for (int i = 0; i < 40; i++) {
            Bitmap bitmap = mediaMetadataRetriever.getFrameAtTime(i * 250000);//1초 영상 추출

            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
            String filename = sdf.format(new Date()) + ".png";  //현재 시간으로 파일명 지정 20210618123456

            StorageReference imgRef = firebaseStorage.getReference(nickname.getText() + "/" + i + ".jpg");   //filename

            //bitmap을 Uri로 변환
            ByteArrayOutputStream bytes = new ByteArrayOutputStream();
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
            String path = MediaStore.Images.Media.insertImage(this.getContentResolver(), bitmap, "IMG_" + System.currentTimeMillis(), null);
            ...
            if (i == 39) {
                uploadTask.addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    @Override
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {            //성공시 토스트 메세지 츌력
                        Toast.makeText(MainActivity.this, "success upload", Toast.LENGTH_SHORT).show();
                    }
                });

                try {
                    mqttAndroidClient.publish("hansung/mobile/act", "att".getBytes(), 0, false);
                    mqttAndroidClient.publish("hansung/mobile/dbupload", nickname.getText().toString().getBytes(), 0, false);
                } catch (MqttException e) {
                    e.printStackTrace();
                }
            }
